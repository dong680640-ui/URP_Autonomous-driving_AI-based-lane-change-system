...
SUB_LEFT_SIDE_LIDAR_OBSTACLE_TOPIC_NAME = "left_side_lidar_obstacle_info"
SUB_RIGHT_SIDE_LIDAR_OBSTACLE_TOPIC_NAME = "right_side_lidar_obstacle_info"
...
class MotionPlanningNode(Node):
    def __init__(self):
        ...
        self.sub_left_side_lidar_obstacle_topic = self.declare_parameter('sub_left_side_lidar_obstacle_topic', SUB_LEFT_SIDE_LIDAR_OBSTACLE_TOPIC_NAME).value
        self.sub_right_side_lidar_obstacle_topic = self.declare_parameter('sub_right_side_lidar_obstacle_topic', SUB_RIGHT_SIDE_LIDAR_OBSTACLE_TOPIC_NAME).value
        # 퍼블리셔
        ...
        self.left_side_lidar_data = None
        self.right_side_lidar_data = None
        ...
        self.left_side_lidar_sub = self.create_subscription(Bool, self.sub_left_side_lidar_obstacle_topic, self.side_left_lidar_callback, self.qos_profile)
        self.right_side_lidar_sub = self.create_subscription(Bool, self.sub_right_side_lidar_obstacle_topic, self.side_right_lidar_callback, self.qos_profile)
    ...
    def left_side_lidar_callback(self, msg: Bool):
        self.left_side_lidar_data = msg.data

    def right_side_lidar_callback(self, msg: Bool):
        self.right_side_lidar_data = msg.data
    ...
    def timer_callback(self):
        ...
        left_side_obs = self.left_side_lidar_data if self.left_side_lidar_data is not None else False
        right_side_obs = self.right_side_lidar_data if self.right_side_lidar_data is not None else False
        # 차선 변수 선언
            ...
            if front_obs and (self.car_data is not None and self.car_data.num_cars != 0):
                ...
                for i in range(self.car_data.num_cars):
                    if lane_x - 100 <= self.car_data.x[i] <= lane_x + 30:
                        current_lane_front_obs = 1
                if current_lane_front_obs: # 같은 차선 전방 차량 감지 시
                    self.current_state = State.PREPARING_TO_CHANGE_LANE # state 전이
        ...
        ## 차선 변경 준비 State
            ...
            if front_obs and self.car_data.num_cars != 0:
                ...
                side_obs = right_side_obs if self.current_lane == 0 else left_side_obs
                side_lane_front_obs = None # 옆 차선 전방 차량만 선택
                for i in range(self.car_data.num_cars):
                    if not (lane_x - 100 <= self.car_data.x[i] <= lane_x + 30):
                        side_lane_front_obs = 1
                # 옆 차선에 차량이 없을 시
                if not rear_obs and not side_obs and not side_lane_front_obs:
                    self.target_lane ^= 1
                    self.current_state = State.CHANGING_LANE # state 전이
            else:
                if self.num_lane_changes == 0:
                    self.current_state = State.NORMAL_DRIVING # state 전이
                else:
                    self.current_state = State.OVERTAKING # state 전이
        ...
        ## 차선 변경 State
            ...
            if self.count > max_count and 0 <= abs(self.slope) <= 15:
                ...
                if self.num_lane_changes == 0:
                    self.current_state = State.NORMAL_DRIVING # 차선 변경을 짝수 번 했다면, 일반 주행 State로 전이
                else:
                    self.current_state = State.OVERTAKING # 차선 변경을 홀수 번 했다면, 추월 State로 전이
            # 차선 변경 진행 중
            else:
                self.count += 1
        ...
        ## 추월 State
            ...
            if front_obs and (current_lane_front_obs is not None and self.car_data.y[current_lane_front_obs] >= 240):
                self.current_state = State.PREPARING_TO_CHANGE_LANE # state 전이
            # 옆 차선에 차량이 없을 시
            elif not rear_obs and not side_obs and not front_obs:
                self.target_lane ^= 1
                self.current_state = State.RETURNING_LANE # state 전이
